          ...
          na.action = stats::na.omit)
{
	...
	# test for confscaling
	confscalinglen = 0
	if (is.null(confscaling) == FALSE) {
		assertVector (confscaling)
		if (length(confscaling) != nrow(x)) {
			print (length(confscaling))
			print (nrow(x))
			stop ("Conformal Scaling must have the same size as the training set!")
		}
		if (any(is.na(confscaling)))
			stop("Conformal Scaling cannot have any NAs!")
		cat (" -- Obtained conformal scaling vector\n")
		confscalinglen = as.integer(length(confscaling))
		cat (" -- Length of conformal scaling vector: ", confscalinglen, "\n")
	}
...
", or specify the correct ", sQuote("type"), " argument.")
    ...
    muteFun({
      cret <- .C ("svmtrain",
                  ...
                  as.double  (alpha),
                  as.integer (alphalen),
                  as.double (confscaling),
                  as.integer (confscalinglen),
          ...
          na.action = stats::na.omit)
{
    ...
    if ((is.null(confscaling) || is.null(testscaling)) == TRUE) {
		if ((is.null(confscaling) && is.null(testscaling)) == FALSE) {
			stop ("For conformal scaling provide both scaling parameters.")
		}
	}
	
	# test for confscaling (=for SVs)
	confscalinglen = 0
	if (is.null(confscaling) == FALSE) {
		assertVector (confscaling)
		if (length(confscaling) != nrow(object$SV)) {
			print (length(confscaling))
			print (nrow(object$SV))
			stop ("Conformal Scaling must have the same size as the number of the support vectors!")
		}
		if (any(is.na(confscaling)))
			stop("Conformal Scaling cannot have any NAs!")
		cat (" -- Obtained conformal scaling vector\n")
	...
	# test for testscaling
	testscalinglen = 0
	if (is.null(testscaling) == FALSE) {
		assertVector (testscaling)
		if (length(testscaling) != nrow(newdata)) {
			print (length(testscaling))
			print (nrow(newdata))
			stop ("Conformal Scaling on Test Set must have the same size as the test set!")
		}
		if (any(is.na(testscaling)))
			stop("Conformal Scaling on Test Set cannot have any NAs!")
		cat (" -- Obtained conformal scaling vector for test data!\n")
		testscalinglen = as.integer(length(testscaling))
	}
    ...
    ret <- .C ("svmpredict",
               ...
               as.integer (object$sparse),
				...
				as.double (testscaling),
				as.integer (testscalinglen),
				as.double (confscaling),
				as.integer (confscalinglen),
