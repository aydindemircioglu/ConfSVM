	       ...
	       char   **error)
{
    ...
    printf(" -- %d length of conformal scaling vector obtained \n", *confscalinglen);
	prob.confscaling = NULL;
	prob.isconfscaling = 0;
	if (*confscalinglen > 0) {
		// stupid, but heck
		prob.isconfscaling = 1;
		if (*confscalinglen != prob.l) {
			printf(" -- %d -- %d\n", *confscalinglen, prob.l);
			printf("### WARNING: length of conformal scaling and training set size do NOT match.\n");
		}
		prob.confscaling = Malloc(double, prob.l);
		for (int i = 0; i < (*confscalinglen); i++) {
			//printf("Using given Conformal Scaling.\n");
			prob.confscaling[i] = confscaling[i];
		}
	}
		  ...
		  double *prob)
{
	...
	if (*confscalinglen == 0) {
		...
		if (*probability && svm_check_probability_model(&m)) {
			...
			printf(" -- CONFSCALEPREDICTION");
			for (i = 0; i < *xr; i++)
				ret[i] = svm_predict_conf(&m, train[i], confscaling, testscaling[i]);
		}
	} 
    ...
    if (*decisionvalues) {
		if (*confscalinglen == 0) {
			...
			for (i = 0; i < *xr; i++)
				svm_predict_values_conf(&m, train[i], confscaling, testscaling[i], dec + i * *nclasses * (*nclasses - 1) / 2);
		}
	}
